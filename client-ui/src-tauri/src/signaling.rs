...
pub async fn start_signaling(ws_url: String, file_dc: std::sync::Arc<tokio::sync::Mutex<Option<std::sync::Arc<::webrtc::data_channel::RTCDataChannel>>>>) -> Result<(), Box<dyn Error + Send + Sync>> {
                ...
                if json.get("sdp").is_some() {
                    ...
                    if type_str == "offer" {
                        ...
                        if let Some(ctx) = current_ctx.take() {
                            println!(
                                "Aborting old video loop & closing previous PeerConnection..."
                            );
                        ...
                        match crate::webrtc::init_webrtc(tx_for_webrtc.clone(), file_dc.clone()).await {
                            Ok(new_ctx) => {
