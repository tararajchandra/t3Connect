...
impl ScreenCapturer {
    pub fn new() -> std::result::Result<Self, Box<dyn Error>> {
        unsafe {
            ...
            println!(
                "Successfully initialized DXGI Desktop Duplication on primary monitor ({}x{})!",
                width, height
            );
    ...
    pub fn capture_frame(&mut self) -> std::result::Result<Vec<u8>, Box<dyn Error>> {
        unsafe {
            ...
            let hr = self
                .duplication
                .AcquireNextFrame(10, &mut frame_info, &mut resource);
...
pub fn create_enigo() -> std::result::Result<Enigo, Box<dyn Error + Send + Sync>> {
    Enigo::new(&enigo::Settings::default()).map_err(|e| Box::new(e) as Box<dyn Error + Send + Sync>)
}
