...
impl HardwareEncoder {
    pub fn new(width: u32, height: u32, fps: u32, bitrate: u32) -> Result<Self> {
        unsafe {
            ...
            if num_attributes == 0 || p_attributes.is_null() {
                return Err(windows::core::Error::from_hresult(windows::core::HRESULT(-1))); // Custom error
            }
    ...
    pub fn encode_nv12(&self, nv12_data: &[u8], timestamp: i64) -> Result<Vec<u8>> {
        unsafe {
            // Create Input Sample
